...
defaultAttributes = originalDefaultAttributes = {
    user: ['sAMAccountName', 'mail', 'lockoutTime', 'whenCreated', 'cn', 'displayName', 'title', 'objectGUID'],
...
function parseRangeAttributes(result, opts, callback) {
    ...
    opts = _.defaults(
        { filter: '(distinguishedName=' + parseDistinguishedName(result.dn) + ')', attributes: queryAttributes },
        opts
    );
...
function getRequiredLdapAttributesForUser(opts) {
    ...
    return _.union(['dn', 'cn'], includeGroupMembershipFor(opts, 'user') ? ['member'] : []);
}
                    ...
                    search.call(self, localOpts, function onSearch(err, members) {
                        ...
                        async.forEach(
                            ...
                            function(member, asyncCallback) {
                                ...
                                if (!member.groupType) {
                                    var user = new User(
                                        pickAttributes(member, (opts || {}).attributes || defaultAttributes.user)
                                    );
                                    self.emit(user);
                                    users.push(user);
                                    asyncCallback();
            ...
            function(item, asyncCallback) {
                if (isGroupResult(item)) {
                    ...
                    var other = pickAttributes(
                        item,
                        (opts || {}).attributes || _.union(defaultAttributes.user, defaultAttributes.group)
                    );
...
ActiveDirectory.prototype.getRootDSE = function getRootDSE(url, attributes, callback) {
    ...
    client.bind('', '', function(err) {
        ...
        client.search('', { scope: 'base', attributes: attributes || ['*'], filter: '(objectClass=*)' }, function(
            err,
            result
        ) {
            if (err) {
